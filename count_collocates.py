...
field_id = 'ddd:11'
reg_ex = r'\b(?:(usa |u\.s\.a|vere{1,2}nigde staten|ameri[k|c]a*)\W+(?:\w+\W+){0,15}?[c|s]igaret[a-z]*|[c|s]igaret[a-z]*\W+(?:\w+\W+){0,15}?(usa |u\.s\.a|ver\. staten|vere{1,2}nigde staten|ameri[k|c]a*))\b'
...
with open(main_file, 'r') as f:
    ...
    for row in reader:
        if datefilter(row[4]) and row[8] == "De Telegraaf":
            ...
            var2 = re.search(reg_ex, row[28], re.IGNORECASE)
            if bool(var) == False and bool(var2) == True:
                filterrows()
